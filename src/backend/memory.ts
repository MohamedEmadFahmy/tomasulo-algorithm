...
let cacheSize: number = userInput.cacheSize; //No Of Blocks
let cacheBlockSize: number = userInput.cacheBlockSize; //Size Of Block
// Global variables

let memorySize: number = 2048;
let cacheMissFlag: boolean = false;
export let cacheHitLatency: number = 0;

// set cycle remaining of load and store instructions to cacheMissPenalty
export let cacheMissPenalty: number = 1;

//memory

export let MainMemory: Memory = {
	memory: new Int8Array(memorySize),
};

export let InstructionMemory = {
...
export let DataCache: SystemCache = {
	cache: [],
};
